...
<%
   ...
   day1   = trim(Request("day1"))
	...
	SQL = SQL & " ( select nvl(SPC_CHK,'N') SPC_CHK from suspc.tbb070 where barcode = a.barcode) SPC_CHK, ITYPE , BRCD , YEAR "
	...
	Else
		...
		YEAR1   = arrData(8,0)
			...
			If BRCD = "R" And (YEAR1 ="W" OR YEAR1 ="X") Then
					dc	= 0  'dc율
			'작지 스타일정보 TBP040
  ...
  ' Response END
   ...
   jeago = trim(kk2("jeago"))
	...
	set db = nothing
			...
			Else
			' 20191104 cho CXL라크로와2(앞자리7)제품의 년도W인 제품은 SPC_CHK와 상관없이 등록가능하게 함.

				If BRCD <> "7" AND SPCCHK ="Y" Then
					close_db
					jObject("error") = "1"
